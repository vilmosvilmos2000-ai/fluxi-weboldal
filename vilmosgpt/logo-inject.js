...
(function(){
  try {
    ...
    st.textContent = [
      ...
      '.new-chat-btn{width:100%;border:1px solid rgba(56,189,248,0.4);background:linear-gradient(135deg,rgba(56,189,248,0.22),rgba(139,92,246,0.22));color:#f8fafc;font-weight:700;font-size:14px;padding:12px 14px;border-radius:12px;cursor:pointer;backdrop-filter:blur(12px)}',
      ...
      '.sidebar-backdrop{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:24;backdrop-filter:blur(3px)}',
      '.sidebar-backdrop.open{display:block}',
      '@media (min-width:961px){.sidebar-left{background:rgba(12,12,18,0.72)!important;backdrop-filter:blur(20px) saturate(1.2)!important;border-right:1px solid rgba(255,255,255,0.08)!important}.menu-toggle{display:none!important}}',
...
(function initSidebarUX(){
  try {
    ...
    var sl = document.getElementById('sidebar-left');
    if (sl && !document.getElementById('new-chat-btn')) {
      ...
      hist.innerHTML = '<div class="panel-title">Beszélgetések</div><div class="chat-history" id="chat-history">' +
        '<button type="button" class="history-item active" data-static="1"><span class="history-dot"></span><span class="history-label">Mai chat</span></button>' +
        '<button type="button" class="history-item" data-static="1"><span class="history-dot"></span><span class="history-label">Chat 1</span></button>' +
        '<button type="button" class="history-item" data-static="1"><span class="history-dot"></span><span class="history-label">Tegnap</span></button>' +
        '<button type="button" class="history-item" data-static="1"><span class="history-dot"></span><span class="history-label">Múlt hét</span></button></div>';
