...
function addTypingMessage() {
  ...
  const bubble = document.createElement('div');
  bubble.className = 'bubble';
  bubble.textContent = (typeof currentMode !== 'undefined' && currentMode === 'research')
    ? 'Mély kutatás: 20+ forrást nézek át...'
    : 'Keresem több forrásban...';
...
function localSmartAnswer(text) {
  ...
  if ((lower.includes('tanul') || lower.includes('megjegyez')) && (lower.includes('hogyan') || lower.includes('tipp') || lower.includes('hatékony') || lower.includes('gyorsan'))) {
    return 'Hatékony tanulás: aktív visszahívás, rövid blokkok, hangos magyarázat, példák, alvás. Mit szeretnél tanulni?';
  }
  if (lower.includes('cpu') && lower.includes('ram')) {
    return 'CPU = agy (számol). RAM = munkaasztal (éppen használt adatok). Kevés RAM → lassulás.';
  }
  if (lower.includes('metakogníció') || lower.includes('metakognicio')) {
    return 'A metakogníció: gondolkodsz a saját gondolkodásodról. Pl. „Értem ezt, vagy csak memorizáltam?”';
  }
  if (lower.includes('kritikus gondolkod')) {
    return 'Kritikus gondolkodás: nem fogadod el azonnal az infót — megvizsgálod a forrást, bizonyítékot, másik oldalt, logikát.';
  }
  if (lower.includes('figyelem') && (lower.includes('elfárad') || lower.includes('koncentr'))) {
    return 'Ha elfárad a figyelem: 25+5 perc, értesítések ki, mozogj, egy feladatra fókuszálj.';
  }
  if (lower.includes('programoz') && (lower.includes('kezdő') || lower.includes('nyelv'))) {
    return 'Kezdőnek a Python a legjobb. Webhez: HTML + CSS, aztán JavaScript.';
  }
  if (lower.includes('segíts') || lower === 'help') {
    return 'Persze! Tudok magyarázni, példát adni, tanulási tervet javasolni, vagy emlékezni. Mondd el, miben segíthetek.';
  }
  return null;
}
...
async function fetchWebAnswer(question) {
  // search-boost.js felülírja 20+ forrással kutatás módban
  const q = question.trim();
  const topic = q.replace(/^(mi az a|mi a|mi az|mit jelent|mi ez a|mi ez|mi az a)\s+/i, '').replace(/[?.!]+$/g, '').trim() || q;
  const sources = [
  ...
  for (const r of results) {
    ...
    if (seen.has(key)) continue; seen.add(key); combined.push(r);
    if (combined.join(' ').length >= 520 || combined.length >= 5) break;
  }
  return combined.join(' ').slice(0, 560);
}

function buildFriendlyReply(webText) {
  const maxL = (typeof currentMode !== 'undefined' && currentMode === 'research') ? 1000 : 420;
  const raw = String(webText || '').trim();
  if (raw.length > 45 && (typeof currentMode !== 'undefined' && currentMode === 'research')) {
    return raw.slice(0, maxL);
  }
  const cleaned = extractUsefulSentences(webText || '', maxL);
...
function fallbackAnswer(msg) {
  ...
  return 'Jó kérdés. Próbáljuk lépésről lépésre: mit értesz már belőle, és hol akadtál el?';
}
...
async function answerUser(text) {
  ...
  if (lower.includes('folytasd') || lower.includes('erről még') || lower === 'és?' || lower === 'tovább') {
    ...
    if (last) return 'Az előző témád ez volt: „' + last.slice(0, 100) + '”. Mondd el, mit szeretnél még erről.';
  }
